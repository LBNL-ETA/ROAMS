...
> Example: Simulated emissions are uniform over `[0 kgh,10 kgh]`, and corresponding simulated productivity is uniform over `[1 mscf/d, 11 mscf/d]` (this is not realistic). Further, there's an exact correspondence between emissions and production, i.e. 0 kgh <-> 1 mscf/d, 1kgh <-> 2mscf/d, etc (this is definitely not realistic). At the same time, the estimated "real" productivity distribution shows there are only lower productivity wells - specifically it suggests a uniform distribution over `[1 mscf/d, 5 mscf/d]`. As such, we will do "stratified sampling" of the simulated emissions by sampling uniformly with replacement from those producing in the range `[1 mscf/d, 5 mscf/d]`, and end up with a uniform distribution of simulated emissions in `[0 kgh, 4 kgh]`. The result of this sampling would be our "stratified sample".
