...
# An error message for probabalistic tests
_probabalistic_errormsg = (
    "This probabilistic test observed an event with a small chance "
    "of occurring under test expectations. If you run the test again "
    "and it fails, you likely have a problem."
)
...
class ROAMSModelTests(TestCase):
    ...
    def test_init(self):
        ...
        self.assertEqual(self.model.loglevel,logging.INFO)
        self.assertEqual(self.model.log.name,"roams.model.ROAMSModel")
        self.assertEqual(self.model.log.level,logging.INFO)
    ...
    def test_make_simulated_sample(self):
        ...
        # Assert each underlying value ends up in each sample
        # And sampled approximately 20% of the time
        # (a practical certainty with 1000 samples from a list of 5 numbers)
        for value in [1,2,3,4,5]:
            
            # Count the appearances in each MC run
            sample_counts = (sim_sample==value).sum(axis=0)
            
            # Assert >130 of this value sampled in each MC run
            # (highly probable outcome)
            self.assertTrue(
                (sample_counts>130).all(),_probabalistic_errormsg
            )
            
            # Assert <270 of this value sampled in each MC run
            # (highly probable outcome)
            self.assertTrue(
                (sample_counts<270).all(),_probabalistic_errormsg
            )

        # According to the central limit theorem (since N=1000 satisfies CLT 
        # eligibility), the probability that any one MC run  results in a 
        # sample mean ≥.18 away from 3 is ~.01%. Across 100 independent MC
        # runs, this is a bit less than 1% chance that at least one iteration 
        # has a mean ≥.18. Perhaps good enough for a unit test.
        self.assertTrue(
            (np.abs(sim_sample.mean(axis=0)-3)<.18).all(),
            "This probabilistic test observed an event that has a ~<1% chance "
            "of occurring under expected operation. If you run the test again "
            "and it fails, you likely have a problem."
        )
    ...
    def test_correction_fn(self):
        ...
        np.random.seed(1)
        em, windnorm = self.model.get_aerial_survey_sample()
    ...
    def test_make_aerial_prod_sample(self):
        ...
        self.model.cfg.n_mc_samples = 1000          # Use 1000 MC samples
        ...
        self.assertEqual(aerial_sample.shape,(10,1000))
        self.assertEqual(pd_sample.shape,(10,1000))
        ...
        # one sample
        self.assertEqual((aerial_sample==35).sum(),494)
        self.assertEqual((aerial_sample==48).sum(),320)
        
        # Assert that the mean is supposed to be based on the given seed.
        self.assertTrue(
            aerial_sample[-2:,:].sum(axis=0).mean(),32.65
        )
    ...
    def test_make_aerial_midstream_sample(self):
        ...
        self.assertEqual(aerial_sample.shape,(2,1000))
        self.assertEqual(pd_sample.shape,(2,1000))
        ...
        self.assertEqual((aerial_sample==210).sum(),249)
        self.assertEqual((aerial_sample==242).sum(),190)
        
        # Assert that the mean is supposed to be based on the given seed.
        self.assertEqual(aerial_sample.sum(axis=0).mean(),98.27)
