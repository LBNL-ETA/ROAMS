...
| "noise_fn" | Either `None` (in which case it will use a normal distribution with a mean of 1.07 and SD of 0.4 based on a study of [Insight M measurements in the New Mexico Permian](https://doi.org/10.1021/acs.est.1c06458)), or a dictionary. If a dictionary, should include a `"name"` key that's the name of a method in the `numpy.random` module. Remaining key:value pairs in the dictionary will be passed as keyword arguments to that method at execution time. The `size=` keyword argument is decided by the code based on the size of sampled aerial emissions - do not provide that argument. The noise will be generated by the method, and applied multiplicatively to the sample emissions. | `{"name":"normal","loc":1.07,"scale":0.4}` | `{"name":"normal","loc":1.0,"scale":1.0}` |
