...
| "cutoff_col" | The name of the column in the `plume_file` plume emissions table that holds a flag for whether or not the plume was cut by the field of view of the survey equipment. If None, the  |code assumes there is none. Defaults to `None`  | `"plume is cut off"` |
