...
| "noise_fn" | Either `None` (in which case no noise is applied to sampled aerial data), or a dictionary. If a dictionary, should include a `"name"` key that's the name of a method in the `numpy.random` module. Remaining key:value pairs in the dictionary will be passed as keyword arguments to that method at execution time. The `size=` keyword argument is decided by the code based on the size of sampled aerial emissions - do not provide that argument. The noise will be generated by the method, and applied multiplicatively to the sample emissions. |  `None` | `{"name":"normal","loc":1.0,"scale":1.0}` |
